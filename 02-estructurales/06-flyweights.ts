...
class LocationFactory {
  ...
  getLocationIcon(type: string): LocationIcon {
    if (!this.icons[type]) {
      console.log(
        `%cCreando una instancia del ícono ${type.toLowerCase()}`,
        COLORS.brown,
      );
      const icon = `image_de_${type.toLowerCase()}.png`;
      this.icons[type] = new LocationIcon(type, icon);
    }
...
function main() {
  ...
  const locations = [
    ...
    new MapLocation(35, 45, factory.getLocationIcon("parque")),
    new MapLocation(5, 20, factory.getLocationIcon("parque")),
    new MapLocation(80, 20, factory.getLocationIcon("hospital")),
    new MapLocation(40, 90, factory.getLocationIcon("escuela")),
  ];
