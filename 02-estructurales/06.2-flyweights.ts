...
import {COLORS} from "../helpers/colors.ts"; // 1. Clase que representa el tipo de bala - BulletType (Flyweight)
...
class BulletTypeFactory {
  ...
  getBulletType(name: string, damage: number, color: string): BulletType {
    ...
    if (!this.bulletTypes[key]) {
      console.log(`%cCreando instancia del BulletType ${name}`, COLORS.brown);
      this.bulletTypes[key] = new BulletType(
        name,
        damage,
        color,
      );
