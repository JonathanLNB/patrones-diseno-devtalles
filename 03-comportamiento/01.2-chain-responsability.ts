...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log(
        `%cSupervisor: %cSolicitud aprobada por \$${amount}.`,
        COLORS.blue,
        COLORS.white,
      );
      return;
    ...
    console.log(
      "%cSupervisor: %cSolicitud no puede ser aprobada, pasa al Manager.",
      COLORS.blue,
      COLORS.white,
    );
    this.next(amount);
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 5000) {
      console.log(
        `%cManager: %cSolicitud aprobada por \$${amount}.`,
        COLORS.green,
        COLORS.white,
      );
      return;
    ...
    console.log(
      "%cManager: %cSolicitud no puede ser aprobada, pasa al Director.",
      COLORS.green,
      COLORS.white,
    );
    this.next(amount);
...
class Director extends BaseApprover {
  override approveRequest(amount: number): void {
    console.log(
      `%cDirector: %cSolicitud aprobada por \$${amount}.`,
      COLORS.violet,
      COLORS.white,
    );
  }
