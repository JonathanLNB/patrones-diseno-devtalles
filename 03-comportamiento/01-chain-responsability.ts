...
class BasicSupport extends BaseHandler {
  override handle(request: string): void {
    if (request === "básico") {
      console.log("%cSoporte básico: Resolvio problema básico", COLORS.green);
      return;
    }
    console.log("Soporte básico: Pasando problema a soporte avanzado");
...
class AdvancedSupport extends BaseHandler {
  override handle(request: string): void {
    if (request === "avanzado") {
      console.log("%cSoporte avanzado: Resolvio problema avanzado", COLORS.brown);
      return;
    }
...
class ExpertSupport extends BaseHandler {
  override handle(request: string): void {
    if (request === "experto") {
      console.log("%cSoporte experto: Resolvio problema experto", COLORS.blue);
      return;
    }
